...
class GravitationalWavePolarizations(Scene):
    def construct(self):
        ...
        f_gw = 1 
        run_time=10
        # Create a grid of dots to represent spacetime points
        grid = VGroup(*[Dot(point) for point in 
                        [x * RIGHT + y * UP for x in np.arange(-3, 3.5, 0.5) for y in np.arange(-3, 3.5, 0.5)]])
        
        # Create a ring of points to visualize the wave's effect on a circular configuration
        ...
        def plus_wave_deformation(mob, t):
            ...
            for i, dot in enumerate(mob):
                ...
                new_x = x_offset * (1 + h_plus(h_zero, iota, f_gw, actual_t)) + center_plus_ring[0]
                new_y = y_offset * (1 - h_plus(h_zero, iota, f_gw, actual_t)) + center_plus_ring[1]
        ...
        def cross_wave_deformation(mob, t):
            ...
            for i, dot in enumerate(mob):
                ...
                # Apply the deformation to the offsets
                new_x = x_offset * 1 + y_offset * h_cross(h_zero, iota, f_gw, actual_t) + center_cross_ring[0]
                new_y = y_offset * 1 + x_offset * h_cross(h_zero, iota, f_gw, actual_t) + center_cross_ring[1]
        ...
        def both_deformations(mob, t):
            ...
            for i, dot in enumerate(mob):
                ...
                new_x = x + x * h_plus(h_zero, iota, f_gw, actual_t) + y * h_cross(h_zero, iota, f_gw, actual_t)
                new_y = y + x * h_cross(h_zero, iota, f_gw, actual_t) - y * h_plus(h_zero, iota, f_gw, actual_t) 

                dot.move_to([new_x, new_y, z])

        # Create animations for the "+" and "×" polarizations
        self.play(UpdateFromAlphaFunc(plus_ring, lambda m, t: plus_wave_deformation(m, t)), run_time=run_time, rate_func=linear)
        self.play(UpdateFromAlphaFunc(cross_ring, lambda m, t: cross_wave_deformation(m, t)), run_time=run_time, rate_func=linear)


        both_ring.set_color(PURPLE)
        self.add(both_ring)
        self.play(UpdateFromAlphaFunc(both_ring, lambda m, t: both_deformations(m, t)), run_time=run_time, rate_func=linear)
